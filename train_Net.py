...
from model.MCFNet import MCFNet
...
parser.add_argument('--rgb_root', type=str, default='./RGB-D dataset/train_data/RGB/',
                    help='the training rgb images root')  # train_dut
parser.add_argument('--depth_root', type=str, default='./RGB-D dataset/train_data/depth/',
                    help='the training depth images root')
parser.add_argument('--gt_root', type=str, default='./RGB-D dataset/train_data/GT/',
                    help='the training gt images root')

parser.add_argument('--val_rgb', type=str, default="./RGB-D dataset/test_data/NLPR/RGB/",
                    help='validate rgb path')
parser.add_argument('--val_depth', type=str, default="./RGB-D dataset/test_data/NLPR/depth/",
                    help='validate depth path')
parser.add_argument('--val_gt', type=str, default="./RGB-D dataset/test_data/NLPR/GT/",
...
parser.add_argument('--save_path', type=str, default="ckps/MCFNet/", help='checkpoint path')
...
def validate(test_dataset, model, epoch, opt):
    ...
    if epoch == 0:
        ...
        if mae < best_mae:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), opt.save_path + 'MCFNet_mae_best.pth', _use_new_zipfile_serialization=False)
